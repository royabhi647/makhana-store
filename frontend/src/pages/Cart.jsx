    ...
    <div className="container" style={{ padding: '40px 24px 80px' }}>
      ...
      {cartItems.length === 0 ? (
        <div className="card glass" style={{ textAlign: 'center', padding: '60px 20px', display: 'flex', flexDirection: 'column', gap: '20px', alignItems: 'center' }}>
          ...
          <button onClick={() => navigateTo('catalog')} className="btn btn-primary" style={{ width: 'fit-content' }}>
            Go to Shop 🛍️
          </button>
        </div>
      ) : (
        <div style={{
          ...
          <div style={{
            ...
            <div className="glass animate-fade" style={{
              ...
              <span style={{ fontSize: '1.25rem' }}>{neededForFreeShipping > 0 ? '🚚' : '🎉'}</span>
              <p style={{ fontSize: '0.9rem', fontWeight: 500, color: neededForFreeShipping > 0 ? 'var(--primary-dark)' : 'var(--secondary)' }}>
              ...
              <div key={item.id} className="card animate-fade" style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                  <div style={{
                    ...
                    <span style={{ fontSize: '1.8rem' }}>{item.isCustom ? '🥣' : '📦'}</span>
                  </div>
                  <div>
                    <h4 style={{ fontSize: '1.05rem', color: 'var(--text-dark)' }}>{item.name}</h4>
                    {item.isCustom ? (
                      <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', maxWidth: '350px' }}>
                      ...
                      <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Premium Flavor | 100g Bag</p>
                    )}
                  </div>
                </div>
                {/* Adjuster controls */}
          ...
          <div className="card glass" style={{
            ...
            <h3 style={{ fontFamily: 'var(--font-title)', fontSize: '1.25rem', color: 'var(--text-dark)', borderBottom: '1px solid var(--border-color)', paddingBottom: '12px' }}>
              Order Summary
            </h3>
